...
libtracer_linked.so: libtracer.c
	$(CC) $(CFLAGS) -shared -lpthread -ldl -o libtracer_linked.so libtracer.c -Wl,--version-script=./utils/liba.map
...
libtracer_dlopened.so: libtracer.c
	$(CC) $(CFLAGS) -shared -lpthread -ldl -o libtracer_dlopened.so libtracer.c -Wl,--version-script=./utils/liba.map
